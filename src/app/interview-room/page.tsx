...
async function getBuiltinCameraId(): Promise<string | undefined> {
  try {
    ...
    const builtIn = cameras.find((camera) => {
...
export default function InterviewRoom() {
  ...
  const [interviewSessionId] = useState<string | null>(() => {
    ...
    return sessionStorage.getItem("interviewSessionId");
  });
  ...
  useEffect(() => {
    ...
    const client = createVideoStreamClient({
      ...
      onError: (error) => {
        ...
        setPredictionError(
          error instanceof Error ? error.message : "Video websocket error",
        );
      },

      onClose: () => {
        if (!cleaned) setIsStartingInterview(false);
      },
    });
    ...
    const startCamera = async () => {
      try {
        ...
        if (err.name === "NotReadableError") message = "Camera already in use";
  ...
  useEffect(() => {
    ...
    try {
      const raw = sessionStorage.getItem("interviewQuestions");
    ...
    window.setTimeout(() => {
      ...
      void startQuestionRef.current(firstQuestion);
    }, 1500);
  }, [interviewSessionId]);
          ...
          >
            ...
            <div
              className="
                ...
                rounded-3xl
              "
              onDoubleClick={() => setIsSwapped((previous) => !previous)}
            >
              {isSwapped ? (
                <div
                  className="
